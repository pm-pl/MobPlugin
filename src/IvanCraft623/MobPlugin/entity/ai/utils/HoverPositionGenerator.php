...
use IvanCraft623\MobPlugin\libs\_834e47082f770344\IvanCraft623\Pathfinder\evaluator\WalkNodeEvaluator;
use IvanCraft623\MobPlugin\libs\_834e47082f770344\IvanCraft623\Pathfinder\world\SyncBlockGetter;
...
/**
 * A utility class for generating random positions for entities to hover towards.
 */
class HoverPositionGenerator {
	 ...
	 */
	public static function getPos(PathfinderMob $entity, int $xzRadius, int $yRadius, float $directionX, float $directionZ, float $angle, int $maxHeight, int $minHeight) : ?Vector3{
		...
		return PositionGenerator::generateRandomPosForEntity($entity, static function() use ($entity, $xzRadius, $yRadius, $directionX, $directionZ, $angle, $minHeight, $maxHeight, $isRestricted) : ?Vector3{
			...
			$pos = PositionGenerator::moveUpToAboveSolid(
				$pos,
				$entity->getRandom()->nextBoundedInt($maxHeight - $minHeight + 1) + $minHeight,
				$entity->getWorld()->getMaxY(),
				static fn(Vector3 $position) : bool => $world->getBlock($position)->isSolid()
			);
